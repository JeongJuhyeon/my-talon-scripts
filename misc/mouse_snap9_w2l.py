...
def narrow_w2l(m):
    for d_word in m["mouseSnapNine.digits"].split(' '):
        mg.narrow(int(digits[d_word]))
        time.sleep(0.1)
...
keymap = {
    "{mouseSnapNine.digits}+": narrow_w2l,
...
def do_start_digits_w2l(m):
    try:
        for d_word in m["mouseSnapNineStarter.digits"].split(' '):
            mg.narrow(int(digits[d_word]))
        ctrl.mouse_move(*mg.pos())
    except KeyError:
        pass
